...
# CNN 모델 정의 ○4
input_layer = Input(shape=(MAX_SEQ_LEN,)) # 입력층
embedding_layer = Embedding(VOCAB_SIZE, EMB_SIZE,
                            input_length=MAX_SEQ_LEN)(input_layer) # 가중치
dropout_emb = Dropout(rate=dropout_prob)(embedding_layer) # 버림층
conv1 = Conv1D(filters=128,
               kernel_size=3,
               padding='valid',
               activation=tf.nn.relu)(dropout_emb) # 합성곱층
...
concat = concatenate([pool1, pool2, pool3]) # 3,4,5gram 이후 합치기
hidden = Dense(128, activation=tf.nn.relu)(concat)
dropout_hidden = Dropout(rate=dropout_prob)(hidden)
logits = Dense(5, name='logits')(dropout_hidden)
predictions = Dense(5, activation=tf.nn.softmax)(logits)  # 출력층

# 모델 생성 ○5
model = Model(inputs=input_layer, outputs=predictions)
model.compile(optimizer='adam',
              loss='sparse_categorical_crossentropy', # sparse_ : 카테고리중에서 희소값이 있을때 사용하면, 정확도 상승에 도움을줌
              metrics=['accuracy'])
...
# 모델 저장
model.save('../intent_model(mine).h5')
print('완료되었습니다.')